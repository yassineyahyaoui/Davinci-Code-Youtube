...
def get_videos(targeted_channel, channel_id, shorts):
    ...
    for video in response["items"]:
        if "videoId" in video["id"]:
            ...
            file_cracked_videos = open(os.path.join("data", targeted_channel, "cracked_videos.csv"), "r", newline="")
            content_cracked_videos = file_cracked_videos.read()
            file_cracked_videos.close()

            if (video["id"]["videoId"] not in content) and (video["id"]["videoId"] not in content_downloaded_videos) and (video["id"]["videoId"] not in content_uploaded_videos) and (video["id"]["videoId"] not in content_cracked_videos):
